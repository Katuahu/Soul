...
def process_attack_command(message):
    try:
        ...
        if target_port in blocked_ports:
            ...
            return
        if duration >= 120:
            bot.send_message(message.chat.id, "*⏳ Maximum duration is 120 seconds.*\n"  # Duration limit message
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @kennyhacks* "  # Contact owner message
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"  # Not approved message
        response += "*Please reach out to the owner for assistance: @kennyhacks* 🙏"  # Contact owner message
    else:
...
def owner_command(message):
    response = (
        ...
        "📩 **Telegram:** https://t.me/KennyPvtHaxx\n\n"
